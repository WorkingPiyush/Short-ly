...
import ShortCodeSuggestions from './ShortCodeSuggestions';
...
function CTAUrlbox() {
    ...
    const handleShorten = async () => {
        ...
        try {
            setLoading(true);
            const response = await createUrl({ originalUrl: url, singleUse, password: passwordProtect ? userPassword : null, expiry: showPassword ? expiry : null, shortCode });
        ...
        <div className="min-h-screen  dark:text-black text-black dark:bg-[#0a0a0a] px-4 py-10 flex items-start justify-center">
            <div className="w-full max-w-xl md:max-w-3xl">
                ...
                dark:disabled:hover:bg-emerald-300 disabled:hover:bg-emerald-500 disabled:text-zinc-900 shrink-0 whitespace-nowrap">
                    ...
                    {/* Advanced options */}
                    <p className="text-[11px] font-medium dark:text-white/30 tracking-widest uppercase mb-5">
                        Advanced options
                    </p>

                    <ShortCodeSuggestions url={url} shortCode={shortCode} setShortCode={setShortCode} showSuggestions={showSuggestions} setShowSuggestions={setShowSuggestions} disabled={Boolean(result) || !url.trim() || loading} />
