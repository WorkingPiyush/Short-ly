    ...
    return (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center">
            <div className="w-170 rounded-lg bg-black p-8 shadow-2xl shadow-gray-900 relative">
                ...
                {/* Heading */}
                <h2 className="text-3xl font-bold dark:text-white text-slate-800 mb-8">
                ...
                <div className="flex gap-6 mb-10 ">
                    {socials.map((item, index) => (
                        <div key={index} className="text-center">
                            <a href={item.url} target="_blank" className="w-20 h-20 border rounded-lg flex items-center justify-center hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer" rel="noopener noreferrer">
                                {item.icon}
                            </a>
                            <p className="mt-3 text-sm dark:text-white text-slate-500">{item.label}</p>
                        </div>
                    ))}
                </div>
                ...
                <div className="border border-emerald-300 rounded-lg flex items-center justify-between overflow-hidden">
                    <input readOnly value={link} className="flex-1 px-5 py-4 text-xl outline-none" />
                    {/* <button onClick={handleCopy} className="m-2 px-5 py-3 bg-slate-100 cursor-pointer dark:bg-slate-700 rounded-md font-semibold hover:bg-slate-200 dark:hover:bg-slate-600" >
                        Copy
                    </button> */}
