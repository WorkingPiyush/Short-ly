...
export default function LinkCard({ link }) {
    ...
    return (
        ...
        ${isExpired ? "bg-white/1.5 border-white/6 hover:border-white/10"
        ...
        >
            ...
            <div className="flex items-start justify-between gap-4 mb-4">
              ...
              ${isExpired ? "text-white/35" : "text-white"}`}
                        style={{ fontFamily: "'Syne', sans-serif" }}
                    >
                        {link.original_url}
                    </p>
                    <div className="flex items-center gap-2">
                        <Link to="#" className={`text-[13px] font-medium transition-colors ${isExpired ? "text-emerald-300/35" : "text-emerald-300 hover:text-emerald-200"}`} >
                            {link.short_url}
                        </Link>
            ...
            ${isExpired
                ...
                <span className="inline-flex items-center gap-1.5 text-[12px] text-white/30">
                    ...
                    {isExpired ? "Expired" : "Expires"}&nbsp;<span className="text-white/50 font-medium">{formatedDate(link.expiry_date)}</span>
                </span>
            </div>
        </div>
    );
